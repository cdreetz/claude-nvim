...
local function log(message)
	vim.fn.writefile({ message }, "/tmp/claude_nvim_debug.log", "a")
end
...
function M.setup(opts)
	...
	if not M.config.api_key then
		error("Claude API key not set. Please set it in the setup function.")
	end
	-- Set up commands
	vim.api.nvim_create_user_command("ClaudeGenerate", M.generate_code, { nargs = 1 })
	vim.api.nvim_create_user_command(
		"ClaudeGenerateWithContext",
		M.generate_code_with_context,
		{ nargs = "+", complete = "file" }
	)
	vim.api.nvim_create_user_command("ClaudeEdit", M.edit_code, { range = true, nargs = "?" })
	--vim.api.nvim_create_user_command("claudeApprove", M.approve_changes, {})
end

-- Helper function to make API calls with streaming
local function call_claude_api_stream(messages, callback)
...
function M.generate_code(opts)
	...
	call_claude_api_stream(messages, function(content)
		if content then
